...
with data_analysis:
    ...
    dex_1=sel_col.selectbox('select the comparison valuse',options=('SEX','EDUCATION','MARRIAGE'))
    
    counts_uni=pd.DataFrame(c_default[dex].value_counts())
    st.bar_chart()
    st.text('male and female count on who defalted')
    Counts_two=pd.pivot_table(data=c_default,index=['default ',dex_1],
